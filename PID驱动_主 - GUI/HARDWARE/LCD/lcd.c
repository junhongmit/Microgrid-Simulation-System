...
{          
	...
	for(i=sy;i<=sy+height-1;i++)
	{
		...
		LCD_WriteRAM_Prepare();     			//开始写入GRAM	  
		for(j=0;j<width;j++)LCD_WR_DATA(color);	//设置光标位置 	    
	} 
}  
//在指定区域内填充指定颜色块			 
//(sx,sy),(ex,ey):填充矩形对角坐标,区域大小为:(ex-sx+1)*(ey-sy+1)   
...
{  
 	...
 	for(i=0;i<height;i++)
	{
		...
		LCD_WriteRAM_Prepare();     //开始写入GRAM
		for(j=0;j<width;j++)LCD->LCD_RAM=color[2*(i*width+j)+8]<<8|color[2*(i*width+j)+1+8];//写入数据 
	}	  
}  
//画线
//x1,y1:起点坐标
...
float StrToReal(char *s)
{
    return atof(s);
}
u16 Round(float val)
{
    return (val-floor(val)> 0.5F) ? (u16)val : (u16)(val+0.5F);
}
void LCD_ShowBigNum(u16 x,u16 y,float num)
{         	
	u8 *st=RealToStr(num);
