...
void checknumber()
{
	...
	if (temp>65) {temp=45;strcpy((char*)edit5->text,(char *)RealToStr(temp));} 
	if (temp<45) {temp=65;strcpy((char*)edit5->text,(char *)RealToStr(temp));} 
}
void checknumber2()
{
	float temp;
	...
	if (Amessage&WM_TOUCHMESSAGE) {
		...
		switch (Wparam) {
			...
			case 109://Enter
				if (tab1->TabShow==0) {
					checknumber();
					Freqtrack->pos=(StrToReal((char*)edit5->text)-45)/20.0F*1000;
			...
			case 1001:
				...
				switch (tab1->TabShow) {
					...
					case 1:
						...
						LCD_ShowString(19,156,90,16,16,"Curr_1A:");
						LCD_ShowString(19,196,90,16,16,"Curr_1B:");
						LCD_ShowString(19,236,90,16,16,"Curr_2A:");
						LCD_ShowString(19,276,90,16,16,"Curr_2B:");
							
						LCD_ShowString(205,156,8,16,16,"A");
						LCD_ShowString(205,196,8,16,16,"A");
						LCD_ShowString(205,236,8,16,16,"A");
						LCD_ShowString(205,276,8,16,16,"A");
					break;
					case 2:
		...
		switch (Lparam) {
			case 201:
				W=Wparam*20.0F*1.0e-3F+45;
				strcpy((char*)edit5->text,(char*)RealToStr(Wparam*20.0F*1.0e-3F+45));//45~65Hz
				UpdateWindow(edit5);
			break;
			case 202:
...
void UpdateWindow(void *window)
{
	...
	for (i=0; i<2; i++)
		if (window==&panel[i]&&panel[i].flag&&panel[i].show) {
			...
			LCD_DrawLine(x+5,y+height/2-5,x+width-5,y+height/2-5);
			LCD_DrawLine(x+5,y+5,x+5,y+height-5);//坐标轴
			LCD_DrawLine(x+width-10,y+height/2-5,x+width-5,y+height/2-5);
			LCD_DrawLine(x+width-10,y+height/2+5,x+width-5,y+height/2-5);
			LCD_DrawLine(x+5,y+5,x,y+10);
			LCD_DrawLine(x+5,y+5,x+10,y+10);
			POINT_COLOR=RGB(255,0,0);
			for (k=0; k<3; k++) {
				if (k==0) POINT_COLOR=RGB(217,83,25);
				else if (k==1) POINT_COLOR=RGB(119,172,48);
				else if (k==2) POINT_COLOR=RGB(0,114,189);
				max=0;
				for (j=0; j<((panel[i].data_sum>width-10)?width-10:panel[i].data_sum); j++) {
					LCD_DrawLine(x+5+j,y+height/2-5-(int)panel[i].data[j][k],x+6+j,y+height/2-5-(int)panel[i].data[j+1][k]);
					if (panel[i].data[j][k]>max) {
						max=panel[i].data[j][k]; where=j;
					}
				}
				if (k==0) strcpy((char*)temps,"AB");
				else if (k==1) strcpy((char*)temps,"BC");
				else if (k==2) strcpy((char*)temps,"CA");
				LCD_ShowString(x+5+where-strlen((char*)temps)*6/2+5,y-5+height/2-Round(max)-12,strlen((char*)temps)*6,14,12,temps);
			}
		}
}
